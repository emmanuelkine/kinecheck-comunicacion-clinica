(() => {
  ...
  if (window.__KINECHECK_OWNED_BUTTONS_FIX__) return;
  window.__KINECHECK_OWNED_BUTTONS_FIX__ = true;

  const SELECTOR = "[data-kc-open-product], #course-grid [data-course]";
  ...
  function ensureOpener() {
    ...
    openerPromise = new Promise((resolve, reject) => {
      const existing = document.querySelector('script[data-kc-open-v6], script[src*="academy-open-v6.js"]');
      if (existing) {
  ...
  function productFor(button) {
    return String(button.dataset.kcOpenProduct || button.dataset.course || "").trim();
  }
