...
function isStaleEvent(
  ...
  // Si Hotmart entrega varios estados con la misma fecha,
  // un bloqueo siempre tiene prioridad sobre una aprobación.
...
serve(async (req) => {
  ...
  const expectedHotTok =
    Deno.env.get("EVIDENCE_HOTMART_HOTTOK")?.trim();

  const expectedProductIdRaw =
    Deno.env.get("EVIDENCE_HOTMART_PRODUCT_ID")?.trim();

  const expectedProductId = Number(expectedProductIdRaw);

  if (
    !expectedHotTok ||
    !expectedProductIdRaw ||
    !Number.isFinite(expectedProductId)
  ) {
    console.error(
      "Missing or invalid EVIDENCE_HOTMART_HOTTOK / EVIDENCE_HOTMART_PRODUCT_ID",
    );
  ...
  const receivedHotTok =
    req.headers.get("x-hotmart-hottok")?.trim() || "";
  ...
  const eventId = String(
    body.id || `${event}:${transactionId}:${email}`,
  ).trim();
  ...
  if (active !== null) {
    ...
    if (currentAccessError) {
      return Response.json(
        { error: currentAccessError.message },
        { status: 500 },
      );
    }
        ...
        .upsert(
          {
            ...
            access_source: isOfficialHotmartTest
              ? "hotmart_test"
              : "hotmart",
