...
test("la portada conserva exactamente ocho testimonios anónimos sin rating numérico", async () => {
  const home = await read("index.html");
  const css = await read("home-public-v1.css");
  const js = await read("kinecheck/site-v5.js");

  assert.equal(count(home, '<article class="kc-testimonial">'), 8, "deben existir exactamente 8 testimonios");
  ...
  assert.equal(count(home, '<strong>Beta tester profesional</strong>') + count(home, '<strong>Beta tester estudiante</strong>'), 8, "solo deben usarse etiquetas beta genéricas");
